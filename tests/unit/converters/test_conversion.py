...
class TestStandalonePiOwnership:
    ...
    def test_generated_python_artifacts_are_excluded_from_all_conversion_outputs(
        ...
        for path in [*generated_paths, *intentional_paths]:
        ...
        assert not [
            path for path in output.rglob("*") if {"__pycache__", ".venv"} & set(path.parts)
        ]
        assert any(path.suffix in {".pyc", ".pyo"} for path in output.rglob("*"))
        pi_owned = load_pi_ownership(output)
        assert not [relative for relative in pi_owned if "__pycache__" in relative.parts]
        assert any(relative.suffix in {".pyc", ".pyo"} for relative in pi_owned)
