...
from ai_config.sync_state import compute_owned_codex_hash, compute_plugin_hash
...
class TestClaudeParser:
    ...
    def test_direct_manifest_component_under_generated_environment_is_rejected(
        self, tmp_path: Path
    ) -> None:
        plugin = tmp_path / "plugin"
        (plugin / ".claude-plugin").mkdir(parents=True)
        (plugin / ".claude-plugin/plugin.json").write_text(
            json.dumps({"name": "plugin", "commands": ".venv/command.md"})
        )
        command = plugin / ".venv/command.md"
        ...
        assert any(
            "excluded generated content" in diagnostic.message for diagnostic in ir.diagnostics
        )

    def test_generated_environment_include_is_rejected_from_hash_and_ir(
        self, tmp_path: Path
    ) -> None:
        plugin = tmp_path / "plugin"
        (plugin / ".claude-plugin").mkdir(parents=True)
        (plugin / ".claude-plugin/plugin.json").write_text(
            json.dumps({"name": "plugin", "skills": "skills/demo/SKILL.md"})
        )
        skill = plugin / "skills/demo/SKILL.md"
        skill.parent.mkdir(parents=True)
        skill.write_text(
            "---\nname: demo\ndescription: demo\nx-ai-config-includes: [.venv/include.md]\n---\nbody\n"
        )
        include = plugin / ".venv/include.md"
        include.parent.mkdir()
        include.write_text("generated include")

        assert compute_plugin_hash(plugin) is not None
        ir = parse_claude_plugin(plugin)

        assert not ir.skills()
        assert any(
            "excluded generated content" in diagnostic.message for diagnostic in ir.diagnostics
        )
...
class TestCodexEmitter:
    ...
    def test_generated_environment_support_file_is_rejected_from_output(
        self, tmp_path: Path
    ) -> None:
        source = tmp_path / "source"
        support = source / ".venv/support.py"
        support.parent.mkdir(parents=True)
        support.write_text("generated support")
        assert compute_plugin_hash(source) is not None
        ir = PluginIR(
            identity=PluginIdentity(plugin_id="mcp-files", name="mcp-files"),
            source_path=source,
            components=[
                McpServer(
                    ...
                    transport=McpTransport.STDIO,
                    command="python",
                    args=["${CLAUDE_PLUGIN_ROOT}/.venv/support.py"],
                )
            ],
        )
        ...
        assert not [file for file in result.files if file.path.name == "support.py"]
        assert any(
            "excluded generated content" in diagnostic.message for diagnostic in result.diagnostics
        )
...
class TestStandalonePiOwnership:
    ...
    def test_generated_python_artifacts_are_excluded_from_all_conversion_outputs(
        ...
        plugin = self._plugin(tmp_path, "alpha-plugin", "alpha")
        skill_dir = plugin / "skills" / "alpha"
        generated_paths = [
            skill_dir / "cli" / "src" / "package" / "__pycache__" / "module.pyc",
            skill_dir / "cli" / "src" / "package" / "bytecode.pyc",
            skill_dir / "cli" / "legacy.pyo",
        ]
        for path in generated_paths:
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"generated bytecode")
        shipped_bytecode = skill_dir / "cli" / "shipped.pyc"
        shipped_bytecode.write_bytes(b"deliberately shipped bytecode")
        skill_md = skill_dir / "SKILL.md"
        skill_md.write_text(
            "---\nname: alpha\ndescription: alpha\n"
            "x-ai-config-includes:\n  - skills/alpha/cli/shipped.pyc\n---\nalpha\n"
        )
        venv_bin = skill_dir / "cli" / ".venv" / "bin"
        venv_bin.mkdir(parents=True)
        python_target = tmp_path / "python"
        python_target.write_bytes(b"generated environment interpreter")
        for name in ("python", "python3", "python3.13"):
            (venv_bin / name).symlink_to(python_target)

        # The full-source sync snapshot and conversion parser skip nested generated venvs.
        assert compute_plugin_hash(plugin) is not None
        ir = parse_claude_plugin(plugin)
        ir_paths = {file.relpath for file in ir.skills()[0].files}
        assert ir_paths.isdisjoint(
            {path.relative_to(skill_dir).as_posix() for path in generated_paths}
        )
        assert shipped_bytecode.relative_to(skill_dir).as_posix() in ir_paths
        ...
        assert not [
            ...
            if {"__pycache__", ".venv"} & set(path.parts)
            or path.name in {generated.name for generated in generated_paths}
        ]
        assert len(list(output.rglob(shipped_bytecode.name))) >= len(targets)
        pi_owned = load_pi_ownership(output)
        assert not [
            relative
            for relative in pi_owned
            if "__pycache__" in relative.parts
            or relative.name in {generated.name for generated in generated_paths}
        ]
        assert any(relative.name == shipped_bytecode.name for relative in pi_owned)

        package = codex_package_spec("alpha-plugin", "1.0.0", output)
        generated_hash = compute_owned_codex_hash(package)
        assert generated_hash is not None

        for path in generated_paths:
            path.unlink()
        (skill_dir / "cli" / "src" / "package" / "__pycache__").rmdir()

        clean_output = tmp_path / "clean-output"
        convert_plugin(plugin, [TargetTool.CODEX], clean_output)
        clean_hash = compute_owned_codex_hash(
            codex_package_spec("alpha-plugin", "1.0.0", clean_output)
        )
        assert generated_hash == clean_hash

        # Only generated .venv directories are skipped; other source symlinks still fail closed.
        (skill_dir / "cli" / "unrelated-link").symlink_to(python_target)
        assert compute_plugin_hash(plugin) is None
        assert any(
            "symlink" in diagnostic.message
            for diagnostic in parse_claude_plugin(plugin).diagnostics
        )
