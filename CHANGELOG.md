...
- Conversion now excludes Python `__pycache__` directories and `*.pyc`/`*.pyo` files from Claude plugin skills before constructing the target-neutral IR. Generated bytecode therefore doesn't enter target output, Codex package hashes, or Pi ownership state.
