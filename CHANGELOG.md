...
- Conversion now excludes Python `__pycache__` contents and undeclared loose `.pyc` or `.pyo` files from Claude plugin skills before constructing the target-neutral IR or conversion-cache fingerprint. Interpreter-generated bytecode therefore doesn't enter target output, Codex package hashes, Pi ownership state, or recurring refresh decisions. Full-tree source digests still guard against stale sources during apply; explicitly included or independently referenced bytecode and target-native files remain significant, and existing locally modified owned output requires deliberate one-time cleanup.
