...
- Conversion now excludes Python `__pycache__` directories and `*.pyc`/`*.pyo` files from Claude plugin skills before constructing the target-neutral IR. Generated bytecode therefore doesn't enter target output, Codex package hashes, or Pi ownership state. Reconciliation also retires previously owned bytecode even when Python regenerated it locally, so existing installations converge without weakening protection for other modified outputs.
