...
- Conversion now excludes Python `__pycache__` directories and `*.pyc`/`*.pyo` files from Claude plugin skills before constructing the target-neutral IR or conversion-cache fingerprint. Generated bytecode therefore doesn't enter target output, Codex package hashes, Pi ownership state, or recurring refresh decisions. Full-tree source digests still guard against stale sources during apply, intentional included, referenced support, and target-native bytecode remains significant, and existing locally modified owned bytecode requires deliberate one-time cleanup.
